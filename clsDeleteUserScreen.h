...
public:
	...
	static void ShowDeleteUserScreen()
	{
		...
		clsScreen::DrawScreenHeader("D E L E T E   U S E R", "SCREEN");
		clsUser User = clsUser::Find(_ReadUserName());
		char Answer = 'n';
		if (!User.IsEmptyUserObject())
		{
			_PrintUserCard(User);
			cout << "\nAre you sure do you want delete the User above ? [Y/N] ";
			cin >> Answer;
			if (tolower(Answer) == 'y')
			{
				cout << "\n" << (User.Delete() ? "Deleted Successfully!" : "couldn't delete this User!");
			}
			else
				cout << "\nCanceled!";
		}
